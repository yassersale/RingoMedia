        ...
        },
        {
          ...
          render: function (notification, type, row, meta) {
            ...
            if (formattedRecord.url && formattedRecord.url !== '#') {
              ...
              if (row.targetNotifiersList?.length > 0) {
                var data = row.notification.data;
                ...
                if (isHTML(message)) {
                  ...
                  _$notificationDetail[randomId] = message;
                  return `<button class="btn btn-secondary btn-sm" onclick="ShowNotificationDetailModal('${randomId}')">${app.localize('ShowHTMLData')}</button>`;
                }                
              }
